...
MIKE IO 1D is specifically tailored for interacting with MIKE 1D modelling files, such as network result files (e.g., `.res1d`) and cross-section files (e.g., `.xns11`). MIKE IO, on the other hand, primarily focuses on n-dimensional data files (like `.dfs0`, `.dfs2`, `.dfsu`) for gridded or mesh-based. MIKE+ modellers will often require both packages to support their workflows.

::: {.callout-tip title="History of MIKE IO 1D" collapse="true"}
MIKE IO 1D was historically part of MIKE IO. Due to fundamental differences of the 1D functionality, as well as objectives of MIKE IO at the time, the 1D-related functionality was split into a sister library: MIKE IO 1D.
:::
...
While MIKE IO 1D includes other features, such as cross sectional data, they are excluded from this module for the purpose of keeping content manageable for a beginner. We encourage you to explore MIKE IO 1D's documentation and examples of these features when you're ready.
...
Quantities exist at network locations, which can be converted into Pandas DataFrames.
```{python}
#| output: false
