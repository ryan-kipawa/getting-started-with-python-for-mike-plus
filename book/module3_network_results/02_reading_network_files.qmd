...
This section guides you through loading network results data (e.g. res1d) into Pandas DataFrames. We take a similar approach as we did with MIKE IO, enabling you to build on previously gained knowledge of DataFrames by expanding its scope to network results.
...
The primary method for converting data from a `Res1D` object (or its subsets) into a pandas DataFrame is `read()`. This is a versatile method that can be called at various levels.
...
Notice the `DataFrame` above includes all reaches and nodes, resulting in many columns. Obtain more manageable DataFrames via calling `read()` on subsets of data by location and/or quantity.
...
Similarly, where `read()` is available, you may export data to other common formats. For example:
