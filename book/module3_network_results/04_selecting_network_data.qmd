...
Purely using DataFrames can be difficult due to large number of columns and column header format. For example, this DataFrame has 495 columns:
...
To select the "Water Level" quantity for node "1" via the `DataFrame`, we need to concatenate that information with ':' as a separator:
```{python}
df[["WaterLevel:1"]]
```
...
Clearly this is not a friendly format. For example, what if I want all quantities of this particular reach? This motivated the fluent-like API illustrated in the previous section.
...
::: {.callout-note}
Selecting quantities with the fluent-like API needs to be done on a location group, or a more specific element. It cannot be done via the top-level `Res1D` object. To do that, it's better to filter on `open()`.
:::
...
It's more efficient to do this on opening the file with `open()`, just like with other subsets. To select time steps when opening the file, use the `time` parameter to specify the start and end bounds.
...
for event in calibration_events:
    ...
    for reach in calibration_points:
        ...
        # save figure to png here using with standard matplotlib functionality...
```